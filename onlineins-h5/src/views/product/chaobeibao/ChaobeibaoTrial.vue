<template>
    <div class="section">
        ...
        <div class="content trial-mod">
            ...
            <div :class="setClass2">
                ...
                <div class="details amnt-mod">
                    <p>基本保额</p>
                    <ul>
                        <li  ref="amntMod" :data-val="item" v-for="(item,key) in amnt" :class="{active:(amntSelected === key)}"
                ...
                <div ref="dutyMod"
                     :class="((item.dutyCode==='5317D2'||item.dutyCode==='5317D3'||item.dutyCode==='5031D1')&&item.choFlag!=='M')?'detailP':'detailP hide'"
                    ...
                    <ul v-if="item.riskCode === '5317' || !item.premAmnt">
                ...
                <!-- <div class="bn-topList">
                    <p>药惠通抗癌特药险是否自动续缴<md-icon name="question" @click.native="showYhtRenew" size="lg"></md-icon></p>
                    <ul>
                        <li data-val="Y" :class="{active:(ifRenew == '2')}"
                            @click="renewHandleChoose($event,'2')">是
                        </li>
                        <li data-val="N" :class="{active:(ifRenew == '1')}"
                            @click="renewHandleChoose($event,'1')">否
                        </li>
                    </ul>
                </div> -->
                <!--投保人豁免时显示-->
				<div ref="dutyMod"
				    ...
				    <ul v-if="item.riskCode === '5319' || !item.premAmnt">
				        <li :data-val="0" :data-dutyCode="item.dutyCode" :data-riskCode="item.riskCode"
				            @click="handleChoose($event,6,0)" class="active">不投保
				        </li>
				        <li :data-key="6" :data-val="0" :data-dutyCode="item.dutyCode"
				            :data-riskCode="item.riskCode"
				            @click="handleChoose($event,6,1)">
				            投保
				        </li>
				    </ul>
				</div>
        ...
        <md-popup v-model="isPopupShow3.bottom" position="bottom" class="insformPopup">
          <insform :insformShow="insformShow" :yhtPrice="yhtPrice" @cancelIns="getCancelIns($event)"/>
        </md-popup>
        ...
        <div class="contentBottom" v-if="insGiftInsuranceManage">
            ...
            <div v-if="ifCpsFlag" class="justDown5317" style="background: #CCCCCC; cursor: none; pointer-events: none;" disabled>
                <p>{{productSaleDate}}</p>
                <p>准时开售</p>
            </div>
            <div v-else class="justDown" @click="immediateInsurance">立即投保</div>
        </div>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.productSaleDate = localStorage.getItem("productSaleDate")
            this.ifCpsFlag = localStorage.getItem("ifCpsFlag") == 'false' ? false : true;
            //初始化生效时间，默认设置为第二天
            let day3 = new Date()
        ...
        updated(){
            if (this.proposal.insuredInfo.age <= 10) {
                if (this.$refs.amntMod[7]) {
                    this.$refs.amntMod[7].className = "be-grey"
                    this.$refs.amntMod[8].className = "be-grey"
                }
            }
        },
        computed: {
        ...
        methods: {
            ...
            insuYearHandleChoose(e, index) {
                ...
                if (this.curRiskCode === '5317') {
                    ...
                    //基本保额重置到第一个选择项
                    console.log(this.tempAmntEvent)
            ...
            __defaultAmnt () {
                for(let i=0;i<this.proposal.policyInfoList.length;i++){                  //---附加险修改位置---//
                    if(this.proposal.policyInfoList[i].dutyCode != "5031D1" && this.proposal.policyInfoList[i].dutyCode != "5909D1"){
                       this.proposal.policyInfoList[i].amnt = "100000";
                       this.oldPolicyInfoList.amnt = this.proposal.policyInfoList[i].amnt;
                       this.amntSelectVal = "100000"
                       this.amntSelected = 0;
                    }
                }
            },
            _defaultAmnt(){
                if(!this.amnt.includes(this.amntSelectVal)  || this.amntSelected > this.amnt.length - 1){
                    ...
                    for(let i=0;i<this.proposal.policyInfoList.length;i++){                  //---附加险修改位置---//
                        if(this.proposal.policyInfoList[i].dutyCode != "5031D1" && this.proposal.policyInfoList[i].dutyCode != "5909D1"){
                           this.proposal.policyInfoList[i].amnt = this.amnt[maxLength];
                           this.oldPolicyInfoList.amnt = this.proposal.policyInfoList[i].amnt;
                        }
                    }
                    ...
                    if(this.amnt[this.amntSelectVal]!== this.amntSelectVal){
                        ...
                        for(let i=0;i<this.proposal.policyInfoList.length;i++){                  //---附加险修改位置---//
                            if(this.proposal.policyInfoList[i].dutyCode != "5031D1" && this.proposal.policyInfoList[i].dutyCode != "5909D1"){
                               this.proposal.policyInfoList[i].amnt = this.amnt[curLength];
                               this.oldPolicyInfoList.amnt = this.proposal.policyInfoList[i].amnt;
                            }
                        }
                ...
                if ((this.proposal.insuredInfo.age <= 10) && (this.proposal.policyInfoList[0].amnt >= "450000")) {
                    this.__defaultAmnt()
                }
                console.log(this.amntSelectVal,this.amntSelected)
            },
            amntHandleChoose(e, index) {
                ...
                if (this.proposal.insuredInfo.age <= 10) {
                    if((index == 7) || (index == 8)){
                        Toast.failed("10岁以下儿童最高支持投保40万基本保额");
                        return false
                    }
                }
                ...
                if(this.curRiskCode === '5317') {
                    this._resetCaleDuty("5317D2");
                    this._resetCaleDuty("5317D3");
                    this._resetCaleDuty("5317D4");
                }
                for(let i=0;i<this.proposal.policyInfoList.length;i++){                  //---附加险修改位置---//
                    if(this.proposal.policyInfoList[i].dutyCode == "5031D1"){
                       this.proposal.policyInfoList[i].amnt = 2000000;
            ...
            handleChoose(e, key, amntKey) {
                ...
                let flagNum = 0
                if (this.$refs.dutyMod[1].getElementsByTagName('li')[1].className == 'active' && this.$refs.dutyMod[2].getElementsByTagName('li')[1].className == 'active') {
                    flagNum = 3;
                ...
                if (flagCreate >= 0) {
                    if (flagNum == 0) {
                        if (val == 0) {
                            ...
                            this._delCaleDuty('5317D2')
                            this._delCaleDuty('5317D3')
                            this._delCaleDuty('5317D4')
                        }
                    }
                } else {
                    if (flagNum == 3) {
                        let temp = Object.assign({}, this.policyInfoList, temp);
                        temp.dutyCode = '5317D4';
                        ...
                        this._delCaleDuty('5317D1')
                        this._delCaleDuty('5317D2')
                        this._delCaleDuty('5317D3')
                    } else if(flagNum == 1){
                        let temp = Object.assign({}, this.policyInfoList, temp);
                        temp.dutyCode = '5317D2';
                        temp.riskCode = riskCode;
                        temp.insuredYear = this.insuYearVal;
                        temp.insuredYearFlag = this.insuredYearFlag;
                        temp.payEndYear = this.payEndYearVal;
            ...
            _resetCaleDuty(dutyCode) {
                ...
                if (flagCreate >= 0) {
                    ...
                    if (dutyCode == '5317D2' || dutyCode == '5317D3' || dutyCode == '5317D4') {
...
//
                        ...
                        this.$nextTick(function(){
                            this.$refs.dutyMod[5].className = "detailP delborder"
							if(this.ifHasYht){
								this.$refs.dutyMod[5].className = "hide"
							}else{
								this.$refs.dutyMod[6].className = "detailP addborder"
							}
            ...
            onDatePickerConfirm(columnsValue) {
                ...
                //超倍保规则校验
                    ...
                    this.$nextTick(() => {
                       if (_age > 10) {
                           if (this.$refs.amntMod[7]) {
                               this.$refs.amntMod[7].className = "be-remove-grey"
                               this.$refs.amntMod[8].className = "be-remove-grey"
                           }
                           this.$refs.amntMod[this.amntSelected].className = "active"
                       }
                    })
                    //重置到第一个选择项
                    this._defaultPayEndYear()
            ...
            showToast(n) {
               if (n === '5909D1') {
                    ...
                    this.toastInfo = '被保险人确诊初次发生“恶性肿瘤”，3年后再次发生恶性肿瘤，我们按基本保额给付第二次恶性肿瘤保险金。'
                    Dialog.alert({content: this.toastInfo})
                } else if(n === '5317D3'){
                    this.toastInfo = '被保险人确诊初次发生“急性心肌梗塞”、“ 脑中风后遗症”、“心脏瓣膜手术”或“主动脉手术”中的无论一种或多种，3年后确诊再次发生该种疾病，按基本保额的50%给付第二次心脑血管特定疾病保险金。'
                    Dialog.alert({content: this.toastInfo})
                } else if(n === '5317D4'){
                    this.toastInfo = '被保险人确诊初次发生“恶性肿瘤”3年后，再次发生恶性肿瘤，我们按基本保额给付第二次恶性肿瘤保险金。被保险人确诊初次发生“急性心肌梗塞”、“ 脑中风后遗症”、“心脏瓣膜手术”或“主动脉手术”中的无论一种或多种，3年后确诊再次发生该种疾病，按基本保额的50%给付第二次心脑血管特定疾病保险金。'
                    Dialog.alert({content: this.toastInfo})
                } else if(n === '5031D1'){
