<template>
    <div class="section">
        ...
        <div class="content trial-mod">
            ...
            <div :class="setClass2">
                <h2>选择投保方案</h2>
                <div class="details" ref="addressMod" v-if="curRiskCode === '1315'">
                    <md-field-item
                        name="area"
                        title="所在城市"
                        arrow="arrow-right"
                ...
                <div ref="dutyMod"
                     :class="((item.dutyCode!=='5305D2'&&item.dutyCode!=='5305D5'&&item.dutyCode!=='5305D6'&&item.dutyCode!=='5305D8'&&item.dutyCode!=='5909D1')&&item.choFlag!=='M')?'detailP':'detailP hide'"
                    ...
                    <ul v-if="item.riskCode === '5305' || !item.premAmnt">
                    ...
                    <ul v-else>
                        <li :data-key="key" :data-val="0" v-if="!item.premAmnt" :data-dutyCode="item.dutyCode"
                            :data-riskCode="item.riskCode"
                            @click="handleChoose($event,key,0)">
                            不投保
                        </li>
                        <li :data-key="key" :data-val="amntItem" v-else
                            v-for="(amntItem,amntKey) in item.premAmnt.split('|')"
                            :data-dutyCode="item.dutyCode"
                            :data-riskCode="item.riskCode"
                            @click="handleChoose($event,key,amntKey+1)">{{ amntItem / 10000 }}万
                        </li>
                    </ul>
                </div>
                ...
                <div v-if="curRiskCode == '5305'">
                   <div ref="dutyMod"
                       ...
                       <ul v-if="item.riskCode === '5305' || !item.premAmnt">
                           <li :data-val="0" :data-dutyCode="item.dutyCode" :data-riskCode="item.riskCode"
                               @click="handleChoose($event,10,0)" class="active">不投保
                           </li>
                           <li :data-key="10" :data-val="0" :data-dutyCode="item.dutyCode"
                               :data-riskCode="item.riskCode"
                               @click="handleChoose($event,10,1)">
                               投保
                           </li>
                       </ul>
                   </div>
                </div>
                <div v-else-if="curRiskCode == '5307'">
                   <div ref="dutyMod"
                        id="delborder"
                        :class="(item.dutyCode==='5909D1'&&item.choFlag!=='M')?'detailP':'detailP hide'"
                        v-if="item.dutyCode==='5909D1'"
                        v-for="(item,key) in dutyList" :data-dutycode="item.dutyCode">
                       <p>{{ item.dutyName }}<md-icon name="question" @click.native="showToast(item.dutyCode)" size="lg"></md-icon></p>
                       <ul v-if="item.riskCode === '5307' || !item.premAmnt">
                           <li :data-val="0" :data-dutyCode="item.dutyCode" :data-riskCode="item.riskCode"
        ...
        methods: {
            ...
            calPAWithRule() {
                ...
                for(let i=0;i<this.proposal.policyInfoList.length;i++){                  //---附加险修改位置---//
                    if(this.proposal.policyInfoList[i].dutyCode == "5031D1"){
                       ...
                       this.proposal.policyInfoList[i].payIntv = "12";
                    }
                }
                this.showWhirl = true
                ...
                this.$request({
                    ...
                    successFn: (data) => {
                        ...
                        for (let j = 0; j < res.length; j++) {
                            ...
                            if (res[j].dutyCode === "5305D1" || res[j].dutyCode === "5305D2" || res[j].dutyCode === "5305D5") {
                        ...
                        let d6_index = this._isCreateDuty("5305D6")
                        if (d6_index > 0) {
                            this.proposal.policyInfoList[d6_index].amnt = d6_amnt
                        }
            ...
            insuYearHandleChoose(e, index) {
                ...
                if (this.curRiskCode === '5305') {
                    ...
                    this._resetCaleDuty("5305D2");//重置康惠保其他两个必须责任 5305D2 5305D5
                    this._resetCaleDuty("5305D5");//重置康惠保其他两个必须责任 5305D2 5305D5
                    //this._resetCaleDuty("5305D6");//重置康惠保其他两个必须责任 5305D2 5305D5
                    //康惠保旗舰版
                    if (this.insuYearVal === 70) {
                        ...
                        } else if (this.proposal.insuredInfo.age >= 31 && this.proposal.insuredInfo.age <= 40) {
                    ...
                    if (this.tempAmntEvent) {
                        ...
                        this._resetCaleDuty("5305D2");//重置康惠保其他两个必须责任 5305D2 5305D5
                        this._resetCaleDuty("5305D5");//重置康惠保其他两个必须责任 5305D2 5305D5
                        this._resetCaleDuty("5305D3");//重置康惠保其他两个必须责任 5305D2 5305D5
                        this._resetCaleDuty("5305D4");//重置康惠保其他两个必须责任 5305D2 5305D5
                    }
                }
                    ...
                    if (val === '20Y') {
                        this.payEndYear = ["5Y", "10Y", "1000Y"];
                    } else if (val === '30Y') {
                        this.payEndYear = ["5Y", "10Y", "20Y", "1000Y"];
                    } else if (val === '50A' || val === '60A') {
                        this.payEndYear = ["5Y", "10Y", "20Y", "30Y", "1000Y"];
                    }
                    //重置到第一个选择项
                    this._defaultPayEndYear()
                    //显示豁免
//                    this.$refs.dutyMod[4].className = 'detailP'
                }
                else if (this.curRiskCode === '1315') {
                    //定惠保特殊处理
                    let _age = this.proposal.insuredInfo.age
                    if (this.insuYearVal === 20) {
                        //保障期限：20年（年龄不在18-50周岁的，不展示20年选项）
                        if (_age >= 18 && _age <= 50) {
                            this.payEndYear = ["5Y", "10Y", "20Y"];
                        } else {
                            this.payEndYear = ["5Y", "10Y", "20Y"];
                        }
                    } else if (this.insuYearVal === 30) {
                        //保障期限：30年（年龄不在18-40周岁的，不展示30年选项）
                        if (_age >= 18 && _age <= 50) {
                            this.payEndYear = ["5Y", "10Y", "20Y", "30Y"];
                        } else {
                            this.payEndYear = ["5Y", "10Y", "20Y", "30Y"];
                        }
            ...
            _defaultPayEndYear() {
                ...
                if (this.curRiskCode === '5307') {
                    ...
                    if (this.payEndYearVal == 1000) {
                        ...
                        this.$refs.dutyMod[4].className = 'detailP hide'
                        this.$refs.hmMod.className = 'contentTop hide'
                        this.$refs.dutyMod[4].getElementsByTagName('li')[0].className = 'active'
                        this.$refs.dutyMod[4].getElementsByTagName('li')[1].className = ''
                    } else {
                        this.$refs.dutyMod[4].className = 'detailP'
                    }
                }
            ...
            payEndYearHandleChoose(e, index) {
                ...
                if (this.curRiskCode === '5307') {
                    //童惠保
                    if (parseInt(e.target.dataset.val) == 1000) {
                        //趸交时，去掉投保人豁免
                        this._delCaleDuty('5909D1')
                        this.$refs.dutyMod[4].className = 'detailP hide'
                        this.$refs.hmMod.className = 'contentTop hide'
...
//                this._resetCaleDuty("5305D6");//重置康惠保其他两个必须责任 5305D2 5305D5
            ...
            amntHandleChoose(e, index) {
                ...
                if (this.curRiskCode === '5305') {
                    ...
                    //童惠保特殊处理，投保附加轻症是，保额是主责任的30%
                    this._resetCaleDuty("5307D2");
                }
                this.calPAWithRule();
            },
            ...
            handleChoose(e, key, amntKey) {
                ...
                //康惠保特殊处理，重新试算保额
                if (dutyCode === '5305D3' || dutyCode === '5305D4') {
                    if (amntKey > 0) {
                        val = this.proposal.policyInfoList[0].amnt * 0.3;
                    }
                }
                //童惠保特殊处理，投保附加轻症是，保额是主责任的30%
                if (dutyCode === '5307D2') {
                    if (amntKey > 0) {
                        val = this.proposal.policyInfoList[0].amnt;
                    }
                }
                //康惠保特殊处理，身故或全残保障金
                ...
                if (dutyCode === '5305D7') {
                    if (amntKey == 1) {
                        //投保
                        this._delCaleDuty("5305D6")
                        this._addCaleDuty("5305D8");
                        //如果被保人年龄大于18岁，则不能投豁免责任
                        if (this.proposal.insuredInfo.age >= 18) {
                            this._delCaleDuty('5909D1')
                            if(this.curRiskCode == "5305"){
                              this.$refs.dutyMod[10].getElementsByTagName('li')[0].className = 'active'
                              this.$refs.dutyMod[10].getElementsByTagName('li')[1].className = 'hide'
                            }else if(this.curRiskCode == "5307"){
                               this.$refs.dutyMod[4].getElementsByTagName('li')[0].className = 'active'
                               this.$refs.dutyMod[4].getElementsByTagName('li')[1].className = 'hide'
                            }
                            this.$refs.hmMod.className = 'contentTop hide'
                            this.proposal.insuredRelationToAppnt = 'SF' //本人
                            ...
                            let appntInfo_age = parseInt(this.proposal.insuredInfo.age) + 19;
                            this.proposal.appntInfo.age = appntInfo_age;
                        }
                    } else {
                        //不投保
                        this._delCaleDuty("5305D8")
                        this._addCaleDuty("5305D6");
                        if (this.proposal.insuredInfo.age >= 18) {
                            //附加投保人豁免控制
                            if(this.curRiskCode == "5305"){
                              this.$refs.dutyMod[10].getElementsByTagName('li')[0].className = 'active'
            ...
            loadRiskInfo() {
                this.$request({
                    ...
                    successFn: (data) => {
                        ...
                        if (this.curRiskCode === '5307') {
                            //童惠保 设置默认值，默认保障期限：20年；缴费年限：一次交清、5年、10年
                            this.payEndYear = ["5Y", "10Y", "1000Y"];
                        }
...
//                                this.proposal.policyInfoList[0].amnt = data.riskInfo.amnt.split('|')[0];
                            ...
                            } else if (item.dutyCode === '5305D2' || item.dutyCode === '5305D5' || item.dutyCode === '5305D6') {
                                //康惠保旗舰版 中症疾病保险金及中症疾病豁免保险费 和 轻症疾病保险金 是必须责任 保额是主责任的50%
                                let temp = Object.assign({}, this.policyInfoList, temp);
                                temp.dutyCode = item.dutyCode;
                                temp.riskCode = item.riskCode;
                                ...
                                temp.payEndYearFlag = this.payEndYearFlagVal
                                temp.payIntv = this.payIntvVal
                                if (item.dutyCode === '5305D5') {
                                    temp.amnt = this.proposal.policyInfoList[0].amnt * 0.3;
                                } else if (item.dutyCode === '5305D2') {
                                    temp.amnt = this.proposal.policyInfoList[0].amnt * 0.5;
                                } else if (item.dutyCode === '5305D6') {
                                    temp.amnt = ""
                                }
                                temp.mult = 1
                                this.proposal.policyInfoList.push(temp)
                        ...
                        if(this.curRiskCode == "5305" || this.curRiskCode == "5307"){
                        ...
                        this.$nextTick(function(){                             //清除医惠通下border
                            if(this.curRiskCode == "5305"){
                               this.$refs.dutyMod[9].className = "detailP delborder"
                               if(this.ifHasYht){
                               	this.$refs.dutyMod[9].className = "hide"
                               }else{
                               	this.$refs.dutyMod[10].className = "detailP addborder"
                               }
                            }else if(this.curRiskCode == "5307"){
                               this.$refs.dutyMod[3].className = "detailP delborder"
                               if(this.ifHasYht){
                               	this.$refs.dutyMod[3].className = "hide"
                               }else{
                        ...
                        //重置被投保试算年龄
                        if (this.curRiskCode === '1315') {
                            //定惠保
                            this.minDate = new Date(this.minOrMaxBirthday(55));//最大年龄
                            this.maxDate = new Date(this.minOrMaxBirthday(18, 2));//最小年龄
                            this.currentDate = new Date(this.minOrMaxBirthday(18, 2));//日期默认年龄
                            this.birthdayVal = this.minOrMaxBirthday(18, 2).replace('/', '-').replace('/', '-');
                        } else if (this.curRiskCode === '5305') {
                            //康惠保旗舰版
                            this.minDate = new Date(this.minOrMaxBirthday(55));//最大年龄
                        ...
                        } else if (this.curRiskCode === '5308') {
                            //百年健惠保重疾医疗保险
                            this.minDate = new Date(this.minOrMaxBirthday(65));//最大年龄
                            this.maxDate = new Date(this.minOrMaxBirthday(1));//最小年龄
                            this.currentDate = new Date(this.minOrMaxBirthday(1));//日期默认年龄
                            this.birthdayVal = this.minOrMaxBirthday(1).replace('/', '-').replace('/', '-');
                            this.proposal.policyInfoList[0].getLimit = 0;
                        } else if (this.curRiskCode === '5307') {
                            //童惠保
                            this.minDate = new Date(this.minOrMaxBirthday(17));//最大年龄
                            this.maxDate = new Date(this.minOrMaxBirthday(28, 1));//最小年龄
                            this.currentDate = new Date(this.minOrMaxBirthday(28, 1));//日期默认年龄
                            this.birthdayVal = this.minOrMaxBirthday(28, 1).replace('/', '-').replace('/', '-');
                            this.proposal.insuredInfo.age = 1;
                            this.proposal.insuredInfo.birthday = this.minOrMaxBirthday(1).replace('/', '-').replace('/', '-');
                            //与投保人关系,不可为空,取值范围:SF:本人;CP:配偶;PR:父母;CH:子女;GP:祖父母;GC:孙子女;EB:哥哥;YB:弟弟;ES:姐姐;YS:妹妹
                            this.proposal.insuredRelationToAppnt = 'CH';
                            //豁免投保人
                            this.appMinDate = new Date(this.minOrMaxBirthday(60));//最大年龄
                            this.appMaxDate = new Date(this.minOrMaxBirthday(18, 2));//最小年龄
            ...
            onDatePickerConfirm(columnsValue) {
                ...
                if (this.curRiskCode === '5305') {
                    ...
                    if (_age > 17) {
                        for (let ii = 0; ii < dutys.length; ii++) {
                            if (dutys[ii].getAttribute('data-dutycode') === "5305D4") {
                                dutys[ii].className = "detailP hide"
                                //重置为不投保
                                dutys[ii].getElementsByTagName('li')[0].className = 'active'
                                dutys[ii].getElementsByTagName('li')[1].className = ''
                            }
                        ...
                        //如果选择就去除该责任
                        this._delCaleDuty("5305D4")

                    } else {
                        for (let ii = 0; ii < dutys.length; ii++) {
                            if (dutys[ii].getAttribute('data-dutycode') === "5305D4") {
                                dutys[ii].className = "detailP";
                            }
                        }
                    }
                    ...
                    if (_age >= 41) {
                        this.insuYear = ["60A", "65A", "70A"];
                    } else if (_age >= 50) {
                        this.insuYear = ["60A", "65A", "70A"];
                    } else {
                        this.insuYear = ["30Y", "60A", "65A", "70A"];
                    }
                    this._defaultInsuYear()
                    ...
                    if (this.defaultAddressLevel === 1) {
                        if (_age >= 18 && _age <= 45) {
                            this.amnt = ["300000", "400000", "500000", "600000", "700000", "800000", "900000", "1000000", "1100000",
                                "1200000", "1300000", "1400000", "1500000", "1600000", "1700000", "1800000", "1900000", "2000000"]
                        } else if (_age >= 46 && _age <= 50) {
                            this.amnt = ["300000", "400000", "500000", "600000", "700000", "800000", "900000", "1000000", "1100000",
                                "1200000", "1300000", "1400000", "1500000"]
                        } else if (_age >= 51 && _age <= 55) {
                            this.amnt = ["300000", "400000", "500000", "600000", "700000", "800000"]
                        }
                        ...
                        if (_age >= 18 && _age <= 45) {
                            ...
                            this.amnt = ["300000", "400000", "500000", "600000", "700000", "800000", "900000", "1000000"]
                        } else if (_age >= 51 && _age <= 55) {
                            this.amnt = ["300000", "400000", "500000"]
                        }
                    } else {
                        if (_age >= 18 && _age <= 45) {
                            this.amnt = ["300000", "400000", "500000", "600000", "700000", "800000", "900000", "1000000"]
