        ...
        <div class="content trial-mod">
            ...
            <div :class="setClass2">
                ...
                <div class="details" ref="payEndYearMod">
                    ...
                    <ul>
                        <li :data-val="item" v-for="(item, key) in payEndYear"
                            ...
                            @click="payEndYearHandleChoose($event,key)">{{ item.replace('Y', '年').replace('1000年', '一次缴清')
                            }}
                        </li>
                    </ul>
                </div>
                <div class="details amnt-mod">
                        <md-input-item
                          title="保险费(请输入1000元的整数倍)"
                          :maxlength="8"
                          placeholder="请输入1000元的整数倍"
                          v-model="premamnt"
                          ref="premamnt"
                          id="premamnt"
                          name="premamnt"
                          @blur="checkPrenamnt(premamnt,$event)"
                        ></md-input-item>
                </div>
                <div ref="dutyMod"
                    ...
                    <ul v-if="item.riskCode === '1318' || !item.premAmnt">
                ...
                <div class="bn-topList" v-if="showYihuitong">
                    <p>自动续保<md-icon name="question" @click.native="showToast('ifrenewFlag')" size="lg"></md-icon></p>
                    <ul>
                        <li
                          data-val="Y"
...
<script>
    ...
    export default {
        ...
        methods: {
            checkPrenamnt(value){
                let val = value.trim()
                if(this.payEndYearSelectVal == "1000Y"){
                  if((val < 5000) || (val % 1000 != 0)){
                      Toast({content: `一次缴清，最低保费5000元且需为1000的整数倍`})
                      this.premamnt = 5000
                  }else{
                      this.premamnt = val
                  }
                  ...
                  if((val < 1000) || (val % 1000 != 0)){
                      Toast({content: `最低保费为1000元且需为1000的整数倍`})
                      this.premamnt = 1000
                  }else{
                      this.premamnt = val
                  }
                }
                for(let i = 0;i < this.proposal.policyInfoList.length; i++){
                    this.proposal.policyInfoList[i].prem = this.premamnt
                }
                this.policyInfoList.prem = this.premamnt
            ...
            calPAWithRule() {
                ...
                for(let i=0;i<this.proposal.policyInfoList.length;i++){
                    ...
                    if(this.proposal.policyInfoList[i].dutyCode == "5909D1"){
                       this.proposal.policyInfoList[i].payIntv = 12;
                    }
                }
                this.showWhirl = true
            ...
            payEndYearHandleChoose(e, index) {
                ...
                if (this.payEndYearVal == 1000 || this.payEndYearVal == 3) {
                    //趸交时，去掉投保人豁免
                    this._delCaleDuty('5909D1')
            ...
            loadRiskInfo() {
                this.$request({
                    ...
                    successFn: (data) => {
                        ...
                        this.proposal.policyInfoList[0].prem = 1000;

                        //重置被投保试算年龄
                        if (this.curRiskCode === '1318') {
                            //鑫惠保
                            this.minDate = new Date(this.minOrMaxBirthday(70));//最大年龄
            ...
            onDatePickerConfirm(columnsValue) {
                ...
                //重置缴费年限的选项
                if(_age <= 60){
                    this.payEndYear = ["3Y", "5Y", "10Y", "1000Y"];
                }else if(_age > 60 && _age <= 65){
                    this.payEndYear = ["3Y", "5Y", "1000Y"];
                }else if(_age > 65){
                    this.payEndYear = ["3Y", "1000Y"];
                }
                //鑫惠保规则校验
                /** ------如果年龄小于18周岁，豁免投保人年龄必须大于被保人18周岁----- **/
                if (_age < 18) {
                    ...
                    for (let ii = 0; ii < dutys.length; ii++) {
                        if (dutys[ii].getAttribute('data-dutycode') === "5031D1") {
                            dutys[ii].getElementsByTagName('li')[0].className = 'active'
                            dutys[ii].getElementsByTagName('li')[1].className = ''
                        }
                    }
