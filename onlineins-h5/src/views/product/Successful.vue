<template>
    <section class="successful-wrap">
        <div :class="setClass">
          ...
          <p>{{ cvalidateText }}</p>
          <p v-if="showOpr">
            <a v-if="showGoHomeBtn" id="hf_successful_gohome" class="red ml24" href="javascript:;" @click="goBackHome">返回首页></a>
            <a v-show="showOrderBtn" id="hf_successful_gorderlist" class="red" :class="{'ml24': showGoHomeBtn}" href="javascript:;" @click="goOrderList">查看保单></a>
          </p>
        </div>

        <div v-if="showOpr" class="visit">
          ...
          <div class="btns">
             <img v-if="rvType==null && isDiaoyanOpen" class="activity_img01" src="@/assets/imgs/investigation/activity01.png" alt="">
        ...
        <div v-if="!showOpr" class="backbtns">
          <div class="btns">
              <button id="hf_successful_gorderlist02" @click="goOrderList">查看保单</button>
          </div>

          <div class="btns">
              <button id="hf_successful_gohome02" class="outline" @click="goBackHome">返回首页</button>
          </div>
        </div>
        <div :class="bannerShow ? 'bottomContentBanner' : 'bottomContent'">
            <div v-if="bannerShow" class=" ">
                <a :href="cpsOrderSuccessImageUrl">
                    <img @click="cpsBannerShence()" :src="cpsOrderSuccessImag">
                </a>
...
<script>
    ...
    export default {
        ...
        computed: {
          ...
          showOpr () {
            return (this.visitFlag == 1 && (!this.isDisabled || !this.isNavBack))
          },
          showGoHomeBtn () {
          ...
          showOrderBtn () {
            // 用户未完成回访 或 产品无回访功能 显示查看保单按钮
            return (this.visitFlag == 1 && this.rvType != null) || this.visitFlag == 0
          }
        },
        data() {
            return {
        ...
        methods: {
            init () {
              ...
              this.secondaryDistributor = localStorage.getItem("cps_secondaryDistributor") ||  this.$router.query('secondaryDistributor') || ''
            ...
            goBackHome () {
              if (this.isBrowser) { // 体内返回首页
                this.backHome()
              } else { // 体外
                if (this.$app.isMiniProgram()&&this.$app.isBnMiniApp()){   //是否在小程序内   直接跳转到首页
                    wx.miniProgram.switchTab({ url: '/pages/home/index' })
                    return false
                } else if (this.cosGoBackUrlflag) { // 渠道链接首页
                  this.goBackCpsHome()
                } else {
                  // 回详情页面
                  location.href = `https://bnonline.aeonlife.com.cn/product/Detail.html?product_id=${this.product_id}&thirdClassDistributor=${this.thirdClassDistributor}&secondaryDistributor=${this.secondaryDistributor}&shareUserCode=`
                }
              }
            },
            onlineVisit () {
            ...
            ifConfirm () {
              let that = this     //   于2019/11/6注释  原因是产品认为不合理逻辑  这个功能是点击原生返回键 弹出是否参与回访调研
              if(this.isBrowser){
                  if(this.openInvestigationConfig){
                      setupWebViewJavascriptBridge(function (bridge) {  //   于2019/117注释  原因是禁止了左上角的返回
                          //导航是否显示分享按钮，分享内容先固定死
            ...
            goOrderList(){
                ...
                location.href = `/externalPay/PolicyDetails.html?orderNo=${orderNumber}`
            },
            clearStorage(name){
