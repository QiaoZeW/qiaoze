    ...
    <div class="section">
        ...
        <div class="content trial-mod">
            ...
            <div :class="setClass2" >
                ...
                <div class="details" ref="payEndYearMod">
                    <p>缴费年限</p>
                    <ul>
                        <li data-val="3" :class="{ active : (payEndYearSelected === '1')}"
                            @click="payEndYearHandleChoose($event,'1')">3年
                        </li>
                        <li data-val="6" :class="{ active : (payEndYearSelected === '2')}"
                            @click="payEndYearHandleChoose($event,'2')">6年
                        </li>
                    </ul>
                </div>
                <md-field class="payblock" id="payblock" v-if="ifShowpay == '0'">
                  <div class="payments">
                    <p>缴费方式</p>
                    <ul>
                      <li data-val="Y" :class="{active:(payway === '2')}" @click="isPaywayChoose($event,'2')">月缴</li>
                      <li data-val="N" :class="{active:(payway === '1')}" @click="isPaywayChoose($event,'1')">年缴</li>
                    </ul>
                  </div>
                </md-field>
                <md-field class="payblock" id="payblock" v-else>
                  <div class="payments">
                    <p>缴费方式</p>
                    <ul>
                      <li v-if="ifShowpay == '1'" data-val="Y" :class="{active:(payway === '2')}">月缴</li>
                      <li v-if="ifShowpay == '2'" data-val="N" :class="{active:(payway === '1')}">年缴</li>
                    </ul>
                  </div>
                </md-field>
            </div>
        ...
        <div class="contentBottom">
            ...
            <p class="bottom1" ref="bottom1">
                <span class="zongHe">￥</span><span class="price">{{ proposalResult}}</span><span v-if="payIntvVal == 12" class="year"> /年</span><span v-else class="year"> /月</span></p>
    ...
    export default {
        ...
        data() {
            return {
                ...
                showWhirl: false,
                marriage: '2',  //社保
                insuYearSelected: "1",//保障期限
                payEndYearSelected: "1",//缴费年限
                toastInfo: '',
                showInfo: false,
                basicDialog: {
                    open: false,
        ...
        methods: {
            ...
            payEndYearHandleChoose(e, index) {
                ...
                this._resetPayEndYear(parseInt(e.target.dataset.val), "Y");
                this._resetInsuYear(parseInt(e.target.dataset.val), "Y");
                this.payEndYearVal = parseInt(e.target.dataset.val)
                this.payEndYearFlagVal = "Y"
                this.insuYearVal = parseInt(e.target.dataset.val)
                this.insuredYearFlagVal = "Y"
            ...
            _resetInsuYear(val, flag) {
                for (let index in this.proposal.policyInfoList) {
                    if (this.proposal.policyInfoList[index].insuredYear) {
                        this.proposal.policyInfoList[index].insuredYear = 6
            ...
            loadRiskInfo() {
                ...
                this.$request({
                    ...
                    successFn: (data) => {
                        ...
                        _this.proposal.policyInfoList[0].riskCode = "5325"
                        _this.proposal.policyInfoList[0].insuredYear = parseInt(data.riskInfo.insuYear.split('|')[0]);
                        _this.proposal.policyInfoList[0].insuredYearFlag = "Y"
                        ...
                        _this.proposal.policyInfoList[0].payEndYearFlag = "Y"
                        _this.proposal.policyInfoList[0].payIntv = this.payIntvVal; //缴费频次
                        _this.proposal.policyInfoList[0].dutyCode = "5325D1";

                        data.dutyList.forEach((item, index) => {
                            if (item.priority === 'M') {//必须责任
                                _this.proposal.policyInfoList[0].insuredYear = parseInt(data.riskInfo.insuYear.split('|')[0]);
                        ...
                        //重置被投保试算年龄
                        _this.minDate = new Date(this.minOrMaxBirthday(60));//最大年龄
                        ...
                        _this.$app.showCustomerIntelligence((data) => {
                            getProdFlag (_this.product_id, (flagData) => {
                                _this.showBXD = data.showBXD
                                // 体内显示或者 体外 客服入口判断显示
                                if (flagData.serviceFlag) {
                                    if(_this.isBrowser || _this.showBXD){
                                        _this.showCustomer = true
                                        _this.$refs.bottom1.style.flex = '.7'
                                    }else{
                                        _this.showCustomer = false
                                        _this.$refs.bottom1.style.flex = '1'
                                    }
                                }
                                _this.monthPay = flagData.monthPay
                                _this.yearPay = flagData.yearPay
                                if(_this.monthPay == '1' && _this.yearPay == '1'){
                                    if(_this.payMent == '0'){
                                       _this.ifShowpay = '0'
                                       _this.payway = '2'
                                       _this.payIntvName = '月缴';
                                       _this.payIntvVal = 1;
                                    }else if(_this.payMent == '1'){
                                       _this.ifShowpay = '1'
                                       _this.payway = '2'
                                       _this.payIntvName = '月缴';
                                       _this.payIntvVal = 1;
                                    }else if(_this.payMent == '2'){
                                       _this.ifShowpay = '2'
                                       _this.payway = '1'
                                       _this.payIntvName = '年缴';
                                       _this.payIntvVal = 12;
                                    }else{
                                       _this.ifShowpay = '0'
                                       _this.payway = '2'
                                ...
                                }else if(_this.monthPay == '1' && _this.yearPay == '0'){
                                    _this.ifShowpay = '1'
                                    _this.payway = '2'
                                    _this.payIntvName = '月缴';
                                    _this.payIntvVal = 1;
                                }else if(_this.monthPay == '0' && _this.yearPay == '1'){
                                    _this.ifShowpay = '2'
                                    _this.payway = '1'
                                    _this.payIntvName = '年缴';
                                    _this.payIntvVal = 12;
                                }
                                _this.proposal.policyInfoList[0].payIntv = _this.payIntvVal
                                _this.calPAWithRule();
                                _this.showWhirl = false
                            })
                        })
            ...
            _birthdayOrder (value){
                ...
                if(this.proposal.policyInfoList.find((item) => (item.dutyCode == '5325D1'))){
